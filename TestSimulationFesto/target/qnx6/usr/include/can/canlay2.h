...
/****************************************************************************
*                                                                          
*   Filename    : canlay2.h    (ported to QNX 6.2)                                                
*   Version     : 4.1                                                 
*   Date        : 2004                                    
...
typedef struct _canpc_ressources_s CANPC_RESSOURCES;
...
typedef struct  _param_struct param_struct;
...
PRAEDECL int MIDDECL 
                 INIPC_initialize_board( unsigned short board);
...
PRAEDECL int MIDDECL CANPC_reset_board(unsigned short board);

//#ifdef WIN32
/* +--- CANPC_set_interrupt_event ------------------------------------------*/
PRAEDECL int MIDDECL CANPC_set_interrupt_event(unsigned short board, struct sigevent * event);
//#endif
/* +--- CANPC_reset_chip         -------------------------------------------*/
PRAEDECL int MIDDECL CANPC_reset_chip(unsigned short board);

/* +--- CANPC_initialize_chip(2) -------------------------------------------*/
#define  CANPC_IC_PARAM_ERR               -1  /* Parameter error            */
PRAEDECL int MIDDECL CANPC_initialize_chip (unsigned short board, int presc,
                               int sjw,
                               int tseg1,
                               int tseg2,
                               int sam);

PRAEDECL int MIDDECL CANPC_initialize_chip2(unsigned short board, int presc,
                               int sjw,
                               int tseg1,
                               int tseg2,
                               int sam);
...
/* +--- CANPC_set_mode(2)        -------------------------------------------*/
PRAEDECL int MIDDECL CANPC_set_mode (unsigned short board, int SleepMode,
                        int SpeedMode);

PRAEDECL int MIDDECL CANPC_set_mode2(unsigned short board, int SleepMode,
                        int SpeedMode);

/* +--- CANPC_set_acceptance(2)  -------------------------------------------*/
PRAEDECL int MIDDECL CANPC_set_acceptance (unsigned short board, unsigned int AccCodeStd,
                              unsigned int AccMaskStd,
                              unsigned long AccCodeXtd,
                              unsigned long AccMaskXtd);

PRAEDECL int MIDDECL CANPC_set_acceptance2(unsigned short board, unsigned int AccCodeStd,
                              unsigned int AccMaskStd,
                              unsigned long AccCodeXtd,
                              unsigned long AccMaskXtd);

/* +--- CANPC_set_output_control(2)  ---------------------------------------*/
PRAEDECL int MIDDECL CANPC_set_output_control (unsigned short board, int OutputControl);

PRAEDECL int MIDDECL CANPC_set_output_control2(unsigned short board, int OutputControl);
...
PRAEDECL int MIDDECL CANPC_initialize_interface(unsigned short board, int ReceiveFifoEnable,
                                   int ReceivePollAll,
...
PRAEDECL int MIDDECL CANPC_define_object (unsigned short board, unsigned long Handle,
                             int * ObjectNumber,
                             int Type,
...
PRAEDECL int MIDDECL CANPC_define_object2(unsigned short board, unsigned long Handle,
                             int  * ObjectNumber,
                             int Type,
                             int ReceiveIntEnable,
                             int AutoRemoteEnable,
...
PRAEDECL int MIDDECL CANPC_define_cyclic (unsigned short board, int ObjectNumber,
                             unsigned int Rate,
                             unsigned int Cycles);

PRAEDECL int MIDDECL CANPC_define_cyclic2(unsigned short board, int ObjectNumber,
                             unsigned int Rate,
                             unsigned int Cycles);

/* +--- CANPC_enable_fifo            ---------------------------------------*/

PRAEDECL int MIDDECL CANPC_enable_fifo(unsigned short board);

/* +--- CANPC_optimize_rcv_speed     ---------------------------------------*/
/* this function has no effect on the CANcard                               */
#define  CANPC_ORS_PARAM_ERR              -1  /* Parameter error            */

PRAEDECL int MIDDECL CANPC_optimize_rcv_speed(unsigned short board);

/* +--- CANPC_enable_dyn_obj_buf     ---------------------------------------*/

PRAEDECL int MIDDECL CANPC_enable_dyn_obj_buf(unsigned short board);
...
PRAEDECL int MIDDECL CANPC_enable_timestamps(unsigned short board);
/* +--- CANPC_enable_fifo_transmit_ack  ------------------------------------*/

PRAEDECL int MIDDECL CANPC_enable_fifo_transmit_ack (unsigned short board);

PRAEDECL int MIDDECL CANPC_enable_fifo_transmit_ack2(unsigned short board);

/* +--- CANPC_get_version            ---------------------------------------*/
/* the parameter hw_version is not used with the CAN-AC2                    */

PRAEDECL int MIDDECL CANPC_get_version(unsigned short board, int  * sw_version, 
                                       int  * fw_version,
                                       int  * hw_version,
                                       int  * license,
                                       int  * can_chip_type);
...
PRAEDECL int MIDDECL CANPC_get_serial_number(unsigned short board, unsigned long  * ser_number);

/* +--- CANPC_start_chip             ---------------------------------------*/

PRAEDECL int MIDDECL CANPC_start_chip(unsigned short board);
...
PRAEDECL int MIDDECL CANPC_send_remote_object (unsigned short board, int ObjectNumber,
                                  int DataLength);

PRAEDECL int MIDDECL CANPC_send_remote_object2(unsigned short board, int ObjectNumber,
                                  int DataLength);

/* +--- CANPC_supply_object_data(2)  ---------------------------------------*/
#define  CANPC_SOD_REQ_OVR                -1  /* request overrun            */    

PRAEDECL int MIDDECL CANPC_supply_object_data (unsigned short board, int  ObjectNumber,
                                  int  DataLength,
                                  unsigned char  * pData);

PRAEDECL int MIDDECL CANPC_supply_object_data2(unsigned short board, int  ObjectNumber,
                                  int  DataLength,
                                  unsigned char  * pData);

/* +--- CANPC_supply_rcv_object_data(2)-------------------------------------*/

PRAEDECL int MIDDECL CANPC_supply_rcv_object_data (unsigned short board, int  ObjectNumber,
                                      int  DataLength,
                                      unsigned char  * pData);

PRAEDECL int MIDDECL CANPC_supply_rcv_object_data2(unsigned short board, int  ObjectNumber,
                                      int  DataLength,
                                      unsigned char  * pData);
...
PRAEDECL int MIDDECL CANPC_send_object (unsigned short board, int  ObjectNumber,
                           int  DataLength);

PRAEDECL int MIDDECL CANPC_send_object2(unsigned short board, int  ObjectNumber,
                           int  DataLength);
...
PRAEDECL int MIDDECL CANPC_write_object (unsigned short board, int  ObjectNumber,
                            int  DataLength,
                            unsigned char  * pData);

PRAEDECL int MIDDECL CANPC_write_object2(unsigned short board, int  ObjectNumber,
                            int  DataLength,
                            unsigned char  * pData);
...
PRAEDECL int MIDDECL CANPC_read_rcv_data (unsigned short board, int  ObjectNumber,
                             unsigned char  * pRCV_Data,
                             unsigned long  * Time);

PRAEDECL int MIDDECL CANPC_read_rcv_data2(unsigned short board, int  ObjectNumber,
                             unsigned char  * pRCV_Data,
                             unsigned long  * Time);
...
PRAEDECL int MIDDECL CANPC_read_xmt_data (unsigned short board, int  ObjectNumber,
                             int  * pDataLength,
                             unsigned char  * pXMT_Data);

PRAEDECL int MIDDECL CANPC_read_xmt_data2(unsigned short board, int  ObjectNumber,
                             int  * pDataLength,
                             unsigned char  * pXMT_Data);
...
PRAEDECL int MIDDECL CANPC_read_ac(unsigned short board, param_struct  * param);

/* +--- CANPC_send_data              ---------------------------------------*/
#define  CANPC_SD_FIFO_FULL               -1  /* transmit fifo full         */    

PRAEDECL int MIDDECL CANPC_send_data (unsigned short board, unsigned long Ident,
                         int Xtd,
                         int DataLength,
                         unsigned char  * pData);

PRAEDECL int MIDDECL CANPC_send_data2(unsigned short board, unsigned long Ident,
                         int Xtd,
                         int DataLength,
                         unsigned char  * pData);
...
PRAEDECL int MIDDECL CANPC_send_remote (unsigned short board, unsigned long Ident,
                           int Xtd,
                           int DataLength);

PRAEDECL int MIDDECL CANPC_send_remote2(unsigned short board, unsigned long Ident,
                           int Xtd,
                           int DataLength);

/* +--- CANPC_get_trigger(2)            ---------------------------------------*/
/* these functions have no effect on the CANcard                               */
PRAEDECL int MIDDECL CANPC_get_trigger (unsigned short board, int  * level);

PRAEDECL int MIDDECL CANPC_get_trigger2(unsigned short board, int  * level);

/* +--- CANPC_set_trigger            ---------------------------------------*/
#define  CANPC_ST_FIFO_FULL               -1  /* transmit fifo full         */    

PRAEDECL int MIDDECL CANPC_set_trigger (unsigned short board, int level);

PRAEDECL int MIDDECL CANPC_set_trigger2(unsigned short board, int level);

/* +--- CANPC_reinitialize           ---------------------------------------*/
PRAEDECL int MIDDECL CANPC_reinitialize(unsigned short board);

/* +--- CANPC_get_time               ---------------------------------------*/
PRAEDECL int MIDDECL CANPC_get_time(unsigned short board, unsigned long  * time);
...
PRAEDECL int MIDDECL CANPC_get_bus_state(unsigned short board, int Can);


/* +--- CANPC_reset_rcv_fifo         ---------------------------------------*/
PRAEDECL int MIDDECL CANPC_reset_rcv_fifo(unsigned short board);

/* +--- CANPC_reset_xmt_fifo         ---------------------------------------*/
PRAEDECL int MIDDECL CANPC_reset_xmt_fifo(unsigned short board);

/* +--- CANPC_reset_lost_msg_counter  --------------------------------------*/
PRAEDECL int MIDDECL CANPC_reset_lost_msg_counter(unsigned short board);

/* +--- CANPC_read_rcv_fifo_level    ---------------------------------------*/
/* returns number of items in receive fifo   */
PRAEDECL int MIDDECL CANPC_read_rcv_fifo_level(unsigned short board);

/* +--- CANPC_read_xmt_fifo_level    ---------------------------------------*/
/* returns number of items in transmit fifo  */
PRAEDECL int MIDDECL CANPC_read_xmt_fifo_level(unsigned short board);

/* +--- CANPC_set_path               ---------------------------------------*/
#define  CANPC_SP_STRING_TOO_LONG        -1  /* path string too long        */    
PRAEDECL int MIDDECL CANPC_set_path(unsigned short board, char  * path);

/* +--- INIPC_close_board            ---------------------------------------*/
PRAEDECL int MIDDECL INIPC_close_board(unsigned short board);
#define  INIPC_CB_ERR                     -1  /* error unlocking rerssources*/    

/* +--- CANPC_enable_error_frame_detection  --------------------------------*/
/* this function has no effect on the CAN-AC2                               */
PRAEDECL int MIDDECL CANPC_enable_error_frame_detection(unsigned short board);

/* +--- CANPC_get_device_id  -----------------------------------------------*/
/* this function has no effect on the CAN-AC2                               */
PRAEDECL int MIDDECL CANPC_get_device_id(unsigned short board, unsigned long *pulDeviceId);
...
/* manuals                                                                   */
PRAEDECL int MIDDECL CANPC_init_signals (unsigned short board, unsigned long ulChannelDirectionMask,
                                         unsigned long ulChannelOutputDefaults);

PRAEDECL int MIDDECL CANPC_read_signals  (unsigned short board, unsigned long *pulChannelRead);

PRAEDECL int MIDDECL CANPC_write_signals  (unsigned short board, unsigned long pulChannelWrite, 
